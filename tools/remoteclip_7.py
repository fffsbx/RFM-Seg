    ...
    def forward(self, images,text_features):
        ...
        for t in range(images.size(0)):
            with torch.no_grad(), torch.cuda.amp.autocast():
                ...
                block_size = 7
                stride = 1
                result = torch.zeros(images.size(0), image_features.shape[1], image_features.shape[2],image_features.shape[3])
                result_features = torch.full((6,image_features.shape[1], image_features.shape[2],image_features.shape[3]),0)
