...
from torchvision.transforms import (Pad, ColorJitter, Resize, FiveCrop, RandomCrop,RandomHorizontalFlip, RandomRotation, RandomVerticalFlip)
import random
...
def image_augment(image, mask, patch_size, mode='train_images', val_scale=1.0):
    image_list = []
    mask_list = []
...
def vaihingen_format(inp):
    ...
    image_list, mask_list = image_augment(image=img.copy(), mask=mask.copy(), patch_size=split_size,mode=mode, val_scale=val_scale)
        ...
        for y in range(0, img.shape[0], stride):
            for x in range(0, img.shape[1], stride):
                ...
                if img_tile.shape[0] == split_size and img_tile.shape[1] == split_size \
                    ...
                    if cf[4] > 0.1 and mode == 'train_images':
                        car_imgs, car_masks = car_aug(image_crop, mask_crop)
                        for i in range(len(car_imgs)):
                            out_img_path = os.path.join(imgs_output_dir,"{}_{}_{}_{}.tif".format(img_filename, m, k, i))
                            cv2.imwrite(out_img_path, car_imgs[i])
                            out_mask_path = os.path.join(masks_output_dir,"{}_{}_{}_{}.png".format(mask_filename, m, k, i))
