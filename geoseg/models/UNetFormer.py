...
class UNetFormer(nn.Module):
    ...
    def forward(self, x,batch_idx=0):
        ...
        # res1 = self.conv_layer1(res1)
        # res2 = self.conv_layer2(res2)
        # res3 = self.conv_layer3(res3)
        # res4 = self.conv_layer4(res4)
        # res4 = torch.cat([res4, result_tensor.cuda().to(torch.float)], dim=1)
        # res4 = self.conv_layer5(res4)
        ...
        if self.training:
            x, ah = self.decoder(clip1.to(torch.float), clip2.to(torch.float), clip3.to(torch.float), clip4.to(torch.float), h, w)
            return x, ah
        else:
            x = self.decoder(clip1.to(torch.float), clip2.to(torch.float), clip3.to(torch.float), clip4.to(torch.float), h, w)
            return x